  ...
  options.base.yubiauth =
    ...
    in
    {
      ...
      notify = mkOption {
        type = bool;
        default = true;
        description = "Enable desktop notifications when yubikey auth requires user interaction";
      };
  ...
  config = mkIf cfg.enable {
    ...
    services.udev.extraRules =
      let
        ...
        runCmd = "${pkgs.su}/bin/su - ${config.base.primaryUser} -c '${lockscript}/bin/lockscript.sh'";
      in
      mkIf cfg.lockOnRemove.enable ''
        # lock the screen when the yubikey is removed
        ACTION=="remove",\
          ENV{ID_BUS}=="usb",\
    ...
    environment.etc."u2f-mappings".text = mkIf (builtins.length cfg.mappings > 0) (
