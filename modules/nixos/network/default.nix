...
let
  ...
  domainHost = head (filter (host: self.nixosConfigurations.${host}.config.network.isHost) realHosts);
in
{
  imports = [
    ./dns.nix
  ...
  config = {
    network = {
      ...
      hostIP =
        if (!cfg.isHost) then self.nixosConfigurations.${domainHost}.config.network.hostIP else cfg.ip;
