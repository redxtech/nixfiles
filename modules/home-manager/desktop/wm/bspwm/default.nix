...
in
{
  ...
  config =
    ...
    lib.mkIf wm.enable {
        ...
        rules =
          let
            ...
            mkRule =
              ...
              let
                sel = if title == "" then class else "${if class == "" then "*" else class}:*:${title}";
              in
              {
                name = sel;
                value = {
          ...
          in
          lib.listToAttrs (map mkRule cfg.wm.rules);
      ...
      xdg.configFile."sxhkd/cheatcheet".text =
        let
          entryToCheatSheet =
            {
              description,
      ...
      desktop = {
        ...
        wm.scripts.wm = {
          ...
          sleep = "${pkgs.coreutils}/bin/sleep 1 && ${pkgs.xorg.xset}/bin/xset dpms force off";
        };
      };
