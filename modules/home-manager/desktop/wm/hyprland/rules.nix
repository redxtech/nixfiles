            ...
            let
              ...
              sel =
                if opts.initialTitle != "" then
                  ...
                  wrapSel "class" class;
              workspace =
                if (wsNum == null) then if ws == "*" then "unset" else "name:${ws}" else "${toString wsNum}";
              # all the rules
            in
            optional (workspace != "name:") "${sel},workspace ${workspace}"
