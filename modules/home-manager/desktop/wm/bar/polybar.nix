...
in
{
  config = lib.mkIf cfg.bspwm.enable {
      ...
      with config.user-theme;
      {
        ...
        settings =
          let
            ...
            runSlurm = "${kittyRun} -o initial_window_width=79c -o initial_window_height=22c ${pkgs.slurm-nm}/bin/slurm -i ${config.desktop.hardware.network.interface}";
          ...
          in
          {
            ...
            "module/todo" = {
              ...
              exec = "${pkgs.todoist}/bin/todoist list --filter 'today & !#daily' | ${pkgs.gawk}/bin/awk '{print substr($0, index($0, $6)) \" -> \" $4}'";
              interval = 60;
            ...
            "module/weather" = {
              ...
              interval = 600;
              exec = ''OPENWEATHER_API_KEY="$(${pkgs.coreutils}/bin/cat ${config.xdg.configHome}/secrets/openweathermap.txt)" ${scrips.bar.weather}/bin/weather-bar -u metric'';

              format = {
                underline = "\${colours.weather}";
                text = "<label>%{A}%{A}%{A}";
                prefix = {
                  text = "%{A1:${scripts.general.wttr}:}%{A3:${runHA} light:}%{A2:${runHA} fan:}󰅟";
    ...
    systemd.user.services.polybar-restart = {
      ...
      Service.ExecStart = "${pkgs.systemd}/bin/systemctl --user restart polybar.service";
