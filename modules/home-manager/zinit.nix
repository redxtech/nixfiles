...
in
{
  options.programs.zsh.zinit = {
    ...
    enableSyntaxCompletionsSuggestions = mkEnableOption "Enable fast-syntax-highlighting, zsh-completions and zsh-autosuggestions.";
  };

  config =
    let
      waitLucidPlugins = builtins.filter (
        plugin:
        plugin.ice == {
          wait = "0";
          lucid = "true";
    ...
    mkIf cfg.enable {
      ...
      ++ optional cfg.enableSyntaxCompletionsSuggestions pkgs.nix-zsh-completions;
      ...
      programs.zsh = {
        ...
        syntaxHighlighting.enable = mkIf cfg.enableSyntaxCompletionsSuggestions false;
              ...
              ${concatStrings (
                map (plugin: ''
                  ${optionalString (plugin.ice != [ ])
                    "zinit ice ${concatStringsSep " " (mapAttrsToList iceToStr (defaultIces // plugin.ice))}"
                  }
                  zinit ${if plugin.snippet then "snippet" else "load"} "${plugin.name}"
                '') otherPlugins
              )}
            ''
          }${
