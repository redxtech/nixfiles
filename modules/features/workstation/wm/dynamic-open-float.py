...
window_name = "Extension: \\(Bitwarden Password Manager\\)"
RULES = [
    Rule(
        [
            Match(title=window_name, app_id="firefox"),
            Match(title=window_name, app_id="firefox-nightly"),
        ]
    ),
...
def float(id: int):
    ...
    # NOTE: this is only for bitwarden
    send({"Action": {"SetWindowWidth": {"id": id, "change": {"SetFixed": 650}}}})
    send({"Action": {"SetWindowHeight": {"id": id, "change": {"SetFixed": 725}}}})
    sleep(0.05)
