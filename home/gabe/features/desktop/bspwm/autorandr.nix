...

{
  programs = {
    autorandr = with lib; {
      ...
      profiles = let
        # TODO: switch to hostname based config
        isDesktop = config.profileVars.primaryMonitor == "DisplayPort-0";
        isLaptop = config.profileVars.primaryMonitor == "eDP-1";
      in {
        desktop = mkIf isDesktop {
          config = {
        ...
        laptop-dual-secondary = mkIf isLaptop {
          config = {
            "${config.profileVars.primaryMonitor}" = {
              mode = "1920x1080";
              rate = "60.00";
